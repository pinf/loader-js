...
Assembler.prototype.assembleProgram = function(uri, callback)
{
    var self = this;
        ...
        program.discoverPackages(function assembler_assembleProgram_lambda_discoverPackages_packageForLocator(locator, callback)
        {
            program.resolveLocator(self, locator, function(locator)
            {
                if (!FILE.exists(locator.location))
                    ...
                    callback(program.ensurePackageForLocator(locator));
            });
 ...
 */
Assembler.prototype.addPackageToProgram = function(program, locator, callback)
{
    var self = this;
    ...
    program.resolveLocator(self, locator, function(locator)
    {
        ...
        var pkg = program.ensurePackageForLocator(locator);
        ...
        pkg.discoverMappings(program, function assembler_assembleProgram_lambda_addPackageToProgram_packageForLocator(locator, callback)
        {
            program.resolveLocator(self, locator, function(locator)
            {
                if (!FILE.exists(locator.location))
