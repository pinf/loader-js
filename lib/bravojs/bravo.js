...
bravojs.makeModuleId = function makeModuleId(relativeModuleDir, moduleIdentifier)
{
  ...
  else
  {
    ...
    if (parts.length==1)
    {
      ...
      {
        id = bravojs.contextForId(relativeModuleDir).resolveRelativeId(relativeModuleDir, moduleIdentifier);
      }
      else
      {
...
bravojs.makeModuleIdFromMapping = function makeModuleIdFromMapping(relativeModuleDir, mapping, moduleIdentifier)
{
  ...
  if (typeof mapping.catalog != "undefined")
  {
      ...
      if (typeof id == "undefined")
        throw new Error("Mapping could not be resolved by plugins: " + mapping.toSource());
  }
  else
  if (typeof mapping.location != "undefined")
  {
    if ((mapping.location.indexOf("./") == 0) || (mapping.location.indexOf("../") == 0))
    {
      ...
      throw new Error("Location format '" + mapping.location + "' not supported for mapping: " + mapping.toSource());
  }
  else
    throw new Error("Invalid mapping: " + mapping.toSource());
...
bravojs.contextForId = function bravojs_contextForId(id)
{
  ...
  if (typeof bravojs.contexts[parts[0]] == "undefined")
    bravojs.makeContext(parts[0]);
  return bravojs.contexts[parts[0]];
}
...
bravojs.makeContext = function bravojs_makeContext(id)
{
   ...
   */
  if (typeof bravojs.contexts[id].uid != "undefiend")
...
bravojs.Context = function bravojs_Context(id)
{
  ...
  else
  {
    ...
    if(req.status == 200)
    {
      /* Store package descriptor for mappings resolver */
      this.descriptor = JSON.parse(req.responseText);
      var self = this;
      require.memoize(id, [], function() { return self.descriptor; });
    }
    else
      throw new Error("Error loading package descriptor from: " + URL);
  }
