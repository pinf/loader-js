...
var boot = exports.boot = function(options)
{
            ...
            if (!adapter) {
	            try {
	                ...
	                if (typeof application !== "undefined" && typeof application.addHttpRequestHandler !== "undefined" &&
	                    require(systemId).platform === "Wakanda")
	                {
	                    adapter = "wakanda";
	                }
	            } catch(e) {}
    ...
    optParser.option("--stdoutLogPath").set().help("Path to file to log stdout output");
    optParser.option("--stderrLogPath").set().help("Path to file to log stderr output");
...
//        return;
    ...
    function run()
    {
    	...
    	if (!API.ENV.ignoreGlobalPINF) {
	    	pinfPackagesPath = "/pinf/packages";
	    	if (!API.FILE.exists(pinfPackagesPath) && API.FILE.exists(API.FILE.dirname(pinfPackagesPath))) {
				API.FILE.mkdirs(pinfPackagesPath, parseInt("0775"));
	    	}
	    }
	    ...
	    var init = function(path)
	    {
	        ...
	        if (cliOptions["sub-path"])
	        {
	        	...
	        	path = path.replace(/\/(program\.json)?$/, "") + "/" + cliOptions["sub-path"];
	        }
    ...
    if (cliOptions.pidfile) {
    	...
    	function preRun()
    	{
    		if (!API.FILE.exists(API.FILE.dirname(cliOptions.pidfile)))
    			API.FILE.mkdirs(API.FILE.dirname(cliOptions.pidfile), parseInt("0775"));
...
//    			if (/No such process/.test(stderr)) {
    ...
    {
		var stack = "";
		if (typeof API === "object" && typeof API.SYSTEM === "object" && typeof API.SYSTEM.formatErrorStack === "function") {
        ...
        if (typeof API === "object" && typeof API.SYSTEM != "undefined" && typeof API.SYSTEM.print != "undefined")
