    ...
    loader.bravojs.module.constructor.prototype.load = function pinf_loader_load(moduleIdentifier, callback)
    {
        ...
        function load(data)
        {
            ...
            try
            {
                ...
                // @see https://github.com/joyent/node/issues/1918
                data = data.replace(/^\uFEFF/, '');
 ...
 */
Sandbox.prototype.ensurePackageForLocator = function(locator, options, callback)
{
	if (!callback) throw new Error("'callback' must be specified!");
    ...
    function finalize(packageId, newLocator)
    {
        ...
        callback(self.packages[packageId]);
    }
    ...
    {
    	var self = this;
    	function proceed(pkg)
    	{
    		self.packages[path] = pkg;

    		// If package has a UID set we also index our packages by it
            // TODO: Add version to key if applicable
            if (typeof self.packages[path].uid != "undefined")
            {
                locator.id = self.packages[path].uid;
                self.packages[self.packages[path].uid] = self.packages[path];
            }

            // If locator has an ID set we also index our packages by it
            if (typeof locator.id != "undefined")
            	self.packages[locator.id] = self.packages[path];

            // Convert mapped module IDs to paths
            if (typeof self.packages[path].normalizedDescriptor.json.modules != "undefined")
            {
                var libDir = self.packages[path].getLibDir();

                var modules = {};
                for (var id in self.packages[path].normalizedDescriptor.json.modules)
                {
                    if (id.charAt(0) == "." || id.charAt(0) == "/")
                        modules[id.replace(/^\./, "")] = self.program.descriptor.augmentLocator(self.packages[path].normalizedDescriptor.json.modules[id], options);
                    else
                        modules["/" + libDir + "/" + id] = self.program.descriptor.augmentLocator(self.packages[path].normalizedDescriptor.json.modules[id], options);
                }
                self.packages[path].normalizedDescriptor.json.modules = modules;
            }
            finalize(path, newLocator);
            return;
    	}
        ...
        {
        	proceed(new PACKAGE.Package(new DESCRIPTORS.Dummy(path, {
                extendsDescriptorJSON: newLocator.descriptor,
                sourceDescriptors: self.program.sourceDescriptors
            }), {
                platform: self.platform
            }));
        }
        ...
        {
        	new DESCRIPTORS.Package(path, {
                extendsDescriptorJSON: newLocator.descriptor,
                sourceDescriptors: self.program.sourceDescriptors
            }, function (descriptor)
            {
            	proceed(new PACKAGE.Package(descriptor, {
                    platform: self.platform
                }));
            });
