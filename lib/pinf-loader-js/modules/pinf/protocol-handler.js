...
var JSGI = exports.JSGI = function(options)
{
    ...
    function protocol(scheme, handler) {
        return PROTOCOL.ProtocolHandler.extend(handler, { scheme: scheme }).new();
    }

    this._protocolHandler = protocol(options.scheme,
    {
...
//                        throw new Error("'content-length' response header not set!");
                ...
                if (typeof response.body == "object" && typeof response.body.forEach != "undefined")
                {
                    ...
                    {
                        upstreamResponse.write(str);
                    });
                }
                else
                if (Array.isArray(response.body))
                {
                    upstreamResponse.write(response.body);
                }
                else
            ...
            {
                upstreamResponse.end("[Internal Error] " + e.message + "\n\n" + e.stack);
