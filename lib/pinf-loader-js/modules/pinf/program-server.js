    ...
    if (typeof route.options.programPath != "undefined")
    {
        function send(sandbox)
        {
            ...
            function initContext(pkgId, modulePathId, callback, trackCollectedModules)
            {
                ...
                var info;
                if ((info = sandbox.program.descriptor.contextFor(pkgId, modulePathId)))
                {
                    ...
                    // we have a context mapped to a package
                    // we need to lookup load contexts in the package descriptor
                    if (typeof info.pkg != "undefined")
                    {
                        var pkg = sandbox.packageForId(info.pkg),
                            contexts = pkg.normalizedDescriptor.getContexts();
                        if (contexts && contexts.load)
                        {
                            if (typeof info.load == "undefined")
                                info.load = {};
                            API.UTIL.deepMerge(info.load, contexts.load);
                        }
                    }
