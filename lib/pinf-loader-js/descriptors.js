...
Descriptor.prototype.load = function(path, create, options, callback)
{
    ...
    function mergeFinal()
    {
        if (options.resolveExtends !== false && typeof self.json["extends"] != "undefined")
        {
			traverseExtends(self.json["extends"], function()
			{
    ...
    if (options.resolveExtends === false)
    {
        mergeFinal();
        return;
    }
    ...
    this.load(path, false, options, function()
    {
        if (typeof self.json.uid != "undefined")
        {
            if (!self.json.uid.match(/^https?:\/\/(.*)\/$/))
                throw self.validationError("Value (" + self.json.uid + ") for property 'uid' is not a valid HTTP(s) URL");
        }
...
var Package = exports.Package = function(path, options, callback)
{
    if (typeof path == "undefined")
        return;
...
WorkspaceSources.prototype.augmentLocator = function(locator)
{
	...
	// Look at the archive or location property to determine if we have a local workspace.
	...
	if (typeof locator.archive === "string" && 
		(m = locator.archive.match(/^https?:\/\/github.com\/([^\/]*)\/([^\/]*)\/?/)) &&
		API.FILE.exists(this.basePath + "/github.com/" + m[1] + "/" + m[2]))
	{
		locator = API.UTIL.deepCopy(locator);
		locator.location = this.basePath + "/github.com/" + m[1] + "/" + m[2];
		return locator;
	}
	else
	...
	if (typeof locator.id !== "undefined" && API.FILE.exists(this.basePath + "/" + locator.id))
	{
	    ...
	    {
	    	var path = locator.path.replace(/\/$/, "");
	    	if (locator.location.substring(locator.location.length-path.length) === path)
	    	{
				delete locator.path;
	    	}
	    }
