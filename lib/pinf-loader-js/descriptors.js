...
function walkContexts(contexts, contextId, context, isProgramContext)
{
    contexts[contextId] = context;
    if (typeof context.load != "undefined")
    {
        for (var id in context.load)
        {
            context.load[id].parent = contextId;
            walkContexts(contexts, id, context.load[id], isProgramContext);
            var parent = contextId;
            contexts[id].parents = [];
            while(true)
            {
                contexts[id].parents.push(parent);
                if (typeof contexts[parent].parent == "undefined")
                    break;
                parent = contexts[parent].parent;
            }
            delete context.load[id].parent;
        }
    }
    if (typeof context.include != "undefined")
    {
        contexts[contextId].include = Object.keys(context.include);
    }
    if (isProgramContext === true && contextId.split("@").length == 1)
        contexts[contextId].pkg = contextId;
}

Program.prototype.contextFor = function(pkgId, moduleId)
{
    if (typeof this.json.contexts == "undefined" || typeof this.json.contexts.top == "undefined")
        return false;
    if (typeof this.contexts == "undefined")
    {
        this.contexts = {};
        for (var id in this.json.contexts.top)
        {
            walkContexts(this.contexts, id, this.json.contexts.top[id], true);
            this.contexts[id].top = true;
        }
    }
    var ids = [
        pkgId,
    ...
    {
        ids.push(ids[1].substring(0, ids[1].length-3));
    }
    for (var i=0,ic=ids.length ; i<ic ; i++ )
    {
        if (typeof this.contexts[ids[i]] != "undefined")
        {
...
Package.prototype.getContexts = function()
{
    if (typeof this.json.contexts == "undefined")
        return false;
    if (typeof this.contexts == "undefined")
    {
        this.contexts = {};
        if (typeof this.json.contexts.load != "undefined")
        {
            this.contexts.load = {};
            for (var id in this.json.contexts.load)
            {
                walkContexts(this.contexts.load, id, this.json.contexts.load[id]);
            }
        }
    }
