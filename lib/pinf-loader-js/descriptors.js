...
function walkContexts(contexts, contextId, context, options)
{
        ...
        for (var id in context.load)
        {
            ...
            if (id.split("@").length == 1 && id.charAt(0) != "/")
            {
                ...
                if (typeof ctx.load != "undefined")
                {
                    ...
                    {
                        if (typeof ctx.load[id2].parents != "undefined")
                            ctx.load[id2].parents = ctx.load[id2].parents.map(function(parent)
                            {
                                if (parent.charAt(0) == "/")
                                    return id + "@" + parent;
                                return parent;
                ...
                contexts[ctx.id] = context.load[id] = API.UTIL.deepMerge(context.load[id], ctx);
            }
            else
            {
                var targetId = id;
                ...
                walkContexts(contexts, targetId, context.load[id], options);
            }
       }
    }
    if (typeof context.include != "undefined" && !Array.isArray(contexts[contextId].include))
    {
        contexts[contextId].include = Object.keys(context.include);
    }
}

function contextFor(descriptor, pkgId, moduleId, options, contexts)
{
    ...
    for (var i=0,ic=ids.length ; i<ic ; i++ )
    {
        ...
        {
            var ctx = descriptor.contexts[ids[i]];
            if (typeof ctx.parent != "undefined")
            {
                var parent = ctx.parent;
                if (typeof ctx.parents == "undefined")
                {
                    ctx.parents = [];
                    while(true)
                    {
                        ctx.parents.push(parent);
                        if (typeof descriptor.contexts[parent].parent == "undefined")
                            break;
                        parent = descriptor.contexts[parent].parent;
                    }
                }
            }
            return ctx;
...
{
    return contextFor(this, pkgId, moduleId, options);
}
